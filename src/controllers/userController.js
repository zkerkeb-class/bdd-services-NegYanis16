...
exports.createUser = async (req, res, next) => {
  try {
    const { nom, prenom, email, password, niveau, classe, authProvider, googleId, avatar } = req.body;
    // Vérifier si l'utilisateur existe déjà
    const existing = await User.findOne({ email });
    if (existing) {
      return res.status(400).json({ error: 'Cet utilisateur existe déjà.' });
    }
    const user = new User({ nom, prenom, email, password, niveau, classe, authProvider, googleId, avatar });
    await user.save();
...
exports.updateTokens = async (req, res, next) => {
  try {
    ...
    if (!user) return res.status(404).json({ error: 'Utilisateur non trouvé.' });
    if (typeof jetons !== 'number') return res.status(400).json({ error: 'Le nombre de jetons doit être un nombre.' });
    switch (operation) {
      case 'add':
        user.jetons = (user.jetons || 0) + jetons;
        break;
      case 'subtract':
        if ((user.jetons || 0) < jetons) return res.status(400).json({ error: 'Jetons insuffisants.' });
        user.jetons -= jetons;
        break;
      case 'set':
        user.jetons = jetons;
